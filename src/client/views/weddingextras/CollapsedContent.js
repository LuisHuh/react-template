...
import { ServiceConsumer, ServiceContext, ServiceProvider } from "@app/ServiceContext";
		...
		<ListView>
			{data.map(({ created_date, content, thumb, path }, key) => (
				...
				<ListOption key={key} avatar>
					...
					<Subtitle className="lg-f20x subtitle-1" size={2}>
...
function CollapsedContent({ isOpen, data }) {
	...
	useEffect(() => {
		if (isOpen) {
			...
			DataSource("getHistoryComments", setComments, idevent_detalle_item);
		}
	}, [isOpen]);
				...
				<Form onSubmit={(data, resetForm) => SaveForm(data, idevent_detalle_item, resetForm)}>
					<Grid type="x" className="grid-padding-x">
						...
						<Cell large="6">
							<FileInput
								buttonLabel="browse"
								label={<UseText i18n="UPLOAD_IMAGE" />}
